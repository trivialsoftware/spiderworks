...
class ContentGenerator {
            ...
            .map((fileObj) =>
            {
                return readFile(path.join(fileObj.root, fileObj.stats.name), 'utf8')
                    ...
                    .tap((page) =>
                    {
                        ...
                        return mkdirp(path.join(sourceDir, outputDir, page.baseURL));
                    })
                    .tap((page) =>
                    {
                        ...
                        return renderMan.render(renderer, page, options)
                            .then((out) =>
                            {
                                ...
                                return writeFile(path.join(sourceDir, outputDir, page.baseURL, page.filename), out);
                            });
                    });
            })
    ...
    generate(sourceDir)
    {
        ...
        // Generate the site
        var outDir = configMan.config.directories.output;
        return Promise.join(this._generateFromMarkdown(sourceDir, outDir),
            this._copyStaticFiles(sourceDir, outDir));
    } // end generate
